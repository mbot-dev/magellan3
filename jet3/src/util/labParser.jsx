...
// CSV spec
const SKIP = parseInt(import.meta.env.VITE_LABTEST_SKIP_CSV); //2;
const ENCODING = import.meta.env.VITE_LABTEST_ENCODING; // "Shift_JIS";
const DEVELOP = Boolean(import.meta.env.VITE_DEVELOP_LABTEST); // false;
...
const parseArray = (arr, fcId, fcName, numDigits) => {
  ...
  const parse = arr.every((item, index) => {
    ...
    switch (index) {
      ...
      case 40: {
        ...
        const test = ABNORMAL_FLAG.some((flg) => {
          if (normalized === flg) {
            obj[key] = normalized;
            return true;
          }
  ...
  // 検査依頼日 をセットするのが必要な場合 => 検体採取日をセット
  if (needsToSetOrderDate) {
    obj["order_date"] = obj["sampling_date"];
  }
  // Order ID がない場合 => Order ID をセット
  if (!hasOrderId) {
    const temp = [];
...
const contentToLabTest = (content, fcId, fcName, numDigits, cb) => {
  ...
  if (DEVELOP) {
    ...
    results.forEach((item) => {
      if (item["pt_id"] !== currentPt) {
        currentPt = item["pt_id"];
        dateShift += 1;
        switch (dateShift) {
          case 0:
            dt = "2024-12-01T08:00:00";
            break;
          case 1:
            dt = "2024-11-01T08:00:00";
            break;
          case 2:
            dt = "2024-10-01T08:00:00";
            break;
          case 3:
            dt = "2024-09-01T08:00:00";
            break;
          default:
            break;
        }
      ...
      item["order_date"] = dt;
      item["sampling_date"] = dt;
      item["test_date"] = dt;
      item["pt_id"] = "T_00001";
      item["pt_name"] = "テスト 患者";
      item["pt_kana"] = "テスト カンジャ";
      item["pt_dob"] = "1980-10-23";
      item["pt_gender"] = "M";
    });
  }
