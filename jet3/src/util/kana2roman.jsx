 ...
 */
export const kanaToRoman = (targetStr, type, options) => {
    /**
     * 変換マップ
     */
    const romanMap = {
    ...
    if (typeof targetStr !== 'string' && typeof targetStr !== 'number') {
        throw new Error('変換する対象が文字列ではありません。');
    }
...
export const toRomanName = (kana) => {
    const roman = kanaToRoman(kana);
    const names = roman.split(' ');
    const family = names[0].substring(0, 1).toUpperCase().toUpperCase();
    const given = names[1].substring(0, 1).toUpperCase().toUpperCase();
    const arr = [];
    arr.push(family);
    arr.push(names[0].substring(1));
    arr.push(' ');
    arr.push(given);
    arr.push(names[1].substring(1));
    return arr.join('');
};
