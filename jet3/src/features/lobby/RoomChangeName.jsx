    ...
    <SystemContent>
      <div className="w3-panel w3-card w3-round">
        ...
        <div className="w3-panel">
          <p>
            {username}
            に、メールを送信しました。メールを開き表示されたリンクをタップしてください。
          </p>
          <p>
            タップするとこの画面は自動的に閉じ、
            {step === "1"
              ? "新しいメールアドレスの入力画面に進みます。"
              : "メールアドレスを変更します。"}
          </p>
          <p>
            メールが届きませんか？
            この変更処理を中止し、最初からやり直してください。
          </p>
        </div>
        <div className="w3-panel">
      ...
      <div className="w3-panel w3-card w3-round">
        <form onSubmit={handleSubmit}>
          ...
          <div className="w3-panel">
            <div
              className="z3-flex"
              style={{ "--gap": "8px", "--justify": "flex-end" }}
            >
              <Buttons
...
const RoomChangeName = () => {
  ...
  useEffect(() => {
    ...
    const asyncPOst = async (payload) => {
      try {
        await margaret.getApi("iss").sendChangeUsernameLink(payload);
  ...
  useEffect(() => {
    ...
    if (pusherEvent === "step1") {
      ...
      dispatch({
        type: "updatePhysician",
        attributes: { username: newUsername },
      });
    }
