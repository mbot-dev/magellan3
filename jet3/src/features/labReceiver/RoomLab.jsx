...
import {saveLabTest} from "../../io/labTestIO";
...
const moduleAttributes = [
    'lab_name', 'pt_id', 'pt_name', 'pt_kana', 'pt_dob', 'pt_gender', 'test_date', 'count', 'createResult'
];

const moduleHeader = [
    'ラボ', '患者ID', '氏 名', 'カ ナ', '生年月日', '性 別', '検査実施日', '検査項目数', '登録結果'
];

const testAttributes = [
    'index',  'lab_name', 'pt_id', 'pt_name', 'pt_kana', 'pt_dob', 'pt_gender',
    'order_date', 'test_date', 'spc_name', 'test_name', 'result_value', 'unit', 'result_status'
];

const testHeader = [
    '番号', 'ラボ', '患者ID', '氏名', 'カナ', '生年月日', '性別', '検査依頼日', '検査実施日', '検体', '検査項目', '結果値', '単位', '状態'
];
                                    ...
                                    <tr key={v4()}>
                                    {
                                        moduleAttributes.map(attr => {
                                            if (attr === 'test_date') {
                                                ...
                                                return (
                                                    <td key={v4()}>{entry['test_items'].length}</td>
                                                );
                                            } else if (attr === 'createResult') {
                                                return (
                                                    <td key={v4()}>{createdResult(row)}</td>
                                                );
                                            } else {
                                                return (
                                                    <td key={v4()}>{entry[attr]}</td>
                                                );
                                            }
                                        })
                                    }
                                ...
                                return (
                                    <tr key={v4()}>
                                    {
                                        testAttributes.map(attr => {
                                            if (attr === 'index') {
                                                return (
                                                    <td key={v4()}>{index + 1}</td>
                                                );
                                            } else if (attr === 'order_date' || attr === 'test_date') {
                                                const sd = labDateString(test[attr], SAMPLING_DF);
                                                return (
                                                    <td key={v4()}>{sd}</td>
                                                );
                                            } else if (attr === 'result_value') {
                                                const val = test['result_value'] ? `${test['result_value']}${test['value_type']}` : '';
                                                return (
                                                    <td key={v4()}>{val}</td>
                                                );
                                            } else {
                                                return (
                                                    <td key={v4()}>{test[attr]}</td>
                                                );
                                            }
                                        })
                                    }
