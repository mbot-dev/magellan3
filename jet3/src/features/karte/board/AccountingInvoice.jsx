...
const AccountingInvoice = ({ facility, patient, karte, date, receiptPivot, mode, amount, onChange, onValidate }) => {
  ...
  useEffect(() => {
    ...
    Object.entries(RECEIPT_AGGREGATE).forEach(([key, value]) => {
      ...
      let bundleTensu = 0;
      karte.r.filter(x => x.group.startsWith(key)).forEach(bundle => {
  ...
  return (
    <Layout>
      <Title>
        ...
        <div style={{ textAlign: 'center', fontSize: '18px' }}>診療費請求書兼領収書</div>
      </Title>
      <Header>
        <HeaderLeft>
          <Label>患者番号</Label>
          ...
          <table className='w3-table w3-border w3-bordered'>
            <tbody>
              ...
              <tr>
                ...
                <td style={TdStyle2}>{hokengai !== 0 ? format(hokengai) : ''}</td>
                <td style={TdStyle3}>円</td>
              </tr>
            </tbody>
          </table>
      ...
      <Footer>
        <Facility style={{ fontSize: '12px' }}>領収書は再発行いたしかねますので、大切に保存してください。</Facility>
        <Facility style={{ fontSize: '12px' }}>厚生労働省が定める診療報酬や薬価等は、医療機関等が仕入れ時に負担する消費税が反映されています。</Facility>
