...
const WhiteQR = ({ user, onCancel }) => {
  ...
  useEffect(() => {
    ...
    const asyncGet = async (data) => {
      try {
        const result = await margaret.getApi("iss").requestWhiteToken(data);
  ...
  useEffect(() => {
    ...
    const pcl = margaret.getApi("pusher");  // .new PusherClient();
      ...
      <ul className="w3-ul">
        <li style={ListStyle}>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <span>
              1.スマートフォンで右のQRコードを撮影し、表示されたURLを開いてください。
            </span>
            <QRBoxSmall src={openUrl} alt={TEXT_ALT} />
          </div>
        </li>
        <li style={ListStyle}>
          2.Webアプリが起動します。撮影ボタンをタップし、被写体を撮影してください。
        </li>
        <li style={ListStyle}>
          3.撮影を終えたら、添付ボタンをタップしてください。QRスキャナが起動しますので、下のQRコードを撮影してください。
        </li>
        <li style={ListStyle}>
          4.画像が挿入されたらこのウインドウは自動的に閉じます。
        </li>
      </ul>
      <ImageBox>
        {qrcode && openUrl ? (
          <div className="w3-panel">
            <QRBox src={qrcode} alt={TEXT_ALT} />
          </div>
        ) : (
          <DotBox>...</DotBox>
        )}
