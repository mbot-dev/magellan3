...
const KarteTab = () => {
  ...
  const [
    { appStatus, patientList, currentPatient, dirtyList, user },
    dispatch,
  ] = useStateValue();
  ...
  const handleChangeKarte = (pt) => {
    if (!disabled) {
      dispatch({ type: "changeKarte", patient: pt });
    }
  };

  const handleCloseKarte = (pt) => {
    dispatch({ type: "closeKarte", patient: pt });
  ...
  return (
    <div className="z3-flex">
      {patientList.map((patient, index) => {
        ...
        const selected = ptId === currentPatient.ptId;
        return (
          <PatientTab
            className={
              selected
                ? "z3-background z3-border-bottom-secondary w3-hover-light-gray"
                : "z3-background z3-border-bottom-surface w3-hover-light-gray"
            }
            key={ptId}
          >
            <NameText
