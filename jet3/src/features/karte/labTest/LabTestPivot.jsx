				...
				<thead>
					<tr>
						{TEST_HEADER.map((attr) => {
							return <th key={attr.key}>{attr.label}</th>;
						})}
					</tr>
				</thead>
				<tbody>
						...
						return (
							<tr key={item.id}>
								{ATTRIBUTES.map((attr, col) => {
									if (attr === "resultValue") {
										return (
											<NumberColumn key={col}>
												{valueWithForm(item)}
											</NumberColumn>
										);
									} else if (attr === "lowerLimit" || attr === "upperLimit") {
										return <NumberColumn key={col}>{item[attr]}</NumberColumn>;
									} else if (attr === "comment") {
										return <td key={col}>{concatComment(item)}</td>;
									} else {
										return <td key={col}>{item[attr]}</td>;
									}
								})}
							</tr>
						);
...
const LabTestPivot = ({ patient }) => {
	...
	useEffect(() => {
		const asyncGet = async (fcId, ptId) => {
			try {
		...
		asyncGet(facility.id, patient.ptId);
	}, [fetch]);
		...
		if (selectedTest) {
			...
			selectedTest.forEach((ent, index) => {
				if (ent.count) {
					...
					const arr = test["testItems"]
						.filter((x) => x["testCode"] === ent["testCode"])
						.map((item) => {
							const {
								samplingDate,
								testName,
	...
	const handleDeleteTest = async () => {
		try {
			await margaret.getApi("labTest").deleteLabTest(testToDelete.id);
			setTestToDelete(null);
			setFetch(fetch + 1); // Re fetch
		} catch (err) {
			console.log(err);
			dispatch({ type: "error", error: err });
		}
			...
			<Board style={{ "--mxh": chartData.length > 1 ? "460px)" : "180px" }}>
				<MyTable>
					<StickyColumn>
						<tr>
							<th
								...
								className={
									selectedColumn === 0
										? settings.appTheme.secondary
										: settings.appTheme.background
								}
								data-item={0}
					...
					<StickyBody>
						...
						{rows.map((row) => {
							const key = row[0]["id"];
							const rowTarget = testResult["testItems"].find(
								(x) => x["testCode"] === row[0]["testCode"]
							);
							...
							return (
								<tr key={key}>
									{ATTRIBUTES.map((attr, col) => {
										...
										if (attr === "resultValue") {
											return (
												<NumberColumn key={colKey} className={color}>
													{valueWithForm(rowTarget)}
												</NumberColumn>
											);
										} else if (attr === "lowerLimit" || attr === "upperLimit") {
											return (
											...
											return (
												<td key={colKey} className={color}>
													{concatComment(rowTarget)}
												</td>
											);
										} else {
											return (
	...
	const render = (ent, col) => {
		...
		const test = resultList[index]["testItems"].find(
			(x) => x["testCode"] === ent["testCode"]
		);
						...
						{rows.map((row, rowNum) => {
							...
							return (
								<tr
									...
									data-item={rowNum}
									onClick={handleSelectTestItem}
								>
									{row.map((ent, col) => {
										return col === 0 ? (
											<FixedColumn key={ent.id}>{render(ent, col)}</FixedColumn>
										) : (
											<td key={ent.id}>{render(ent, col)}</td>
										);
									})}
								</tr>
							);
		...
		<div className="w3-panel">
			...
			<div style={{ display: "none" }}>
				<div ref={componentRef}>
					<PrintComponent
						test={selectedColumn !== -1 ? resultList[selectedColumn] : null}
					/>
				</div>
			</div>
			{testToDelete && (
