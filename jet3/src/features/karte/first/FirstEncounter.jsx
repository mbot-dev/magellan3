...
import {
  ENTITY_SPEC_ALLERGY,
  ENTITY_SPEC_BLOOD_TYPE,
  ENTITY_SPEC_CHILDHOOD,
  ENTITY_SPEC_FAMILIY_HISTORY,
  ENTITY_SPEC_INFECTION,
  ENTITY_SPEC_LIFE_STYLE,
  ENTITY_SPEC_PAST_HISTORY,
  ENTITY_SPEC_VACCINATION,
  FIRST_ENCOUNTER_ENTRIES,
} from "./firstSpec";
...
const FirstEncounter = ({ patient }) => {
  ...
  useEffect(() => {
    ...
    const promises = arr.map((entry) => {
      return margaret.getApi("risk").getEntities(entry.entity, fc_id, pt_id);
    });
  ...
  useEffect(() => {
    ...
    FIRST_ENCOUNTER_ENTRIES.filter((x) => x.default === undefined).forEach(
      (entry) => {
