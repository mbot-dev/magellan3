...
const SignUp = () => {
    ...
    const handleSendLink = (email) => {
        const asycSend = async (mail) => {
            try {
                const res = await sendSignupLink({ sub: mail });
                // auth server response
                const { username, channel } = res;
                // Listen to the pusher channel
                dispatch({ type: 'setLoginName', username: username, channel: channel });
                // Show waiting dialog
    ...
    return (
        <Wrapper>
            {stage === STAGE_DIALOG && <SignUpDialog onSendLink={handleSendLink} onCancel={handleCancelSignUp}/>}
            {stage === STAGE_WAITING && <SignUpWaiting loginName={loginName} onCancel={handleCancelWaiting} onResend={handleResend}/>}
            {stage === STAGE_REGISTER && <UserRegistration username={loginName} onRegister={handleRegister} onCancel={handleCancelRegister}/>}
        </Wrapper>
    );
};
