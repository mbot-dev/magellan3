...
import { getAMSimbol, getPMSimbol, HOLIDAYS, irregularInfo, isHoliday, isIrregular, OFF_ENTRIES, setupRelations, WEEK_DAYS } from "./timeModel";
        ...
        switch (action.type) {
            ...
            case 'applyRegularTime': {
                ...
                draft.timeTable.weekDays.forEach(t => {
                    t[timeName] = draft.timeTable[`regular${timeName.charAt(0).toUpperCase()}${timeName.slice(1)}`];
                });
                break;
            }
            case 'holidayCheck': {
                ...
                if (!checked) {
                    target = draft.timeTable.holidays.find(x => x.name === day);
                    if (target) {
                        const index = draft.timeTable.holidays.findIndex(x => x.name === day);
            ...
            case 'setTime': {
                ...
                const index = draft.timeTable.weekDays.findIndex(x => x.name === newTime.name);
    ...
    return timeTable && (
        <Wrapper>
            <div className="z3-flex-column" style={{'--gap': '4px'}}>
                ...
                <div className="z3-flex" style={{'--align': 'center', '--gap': '32px'}}>
                    {
            ...
            <div className="z3-flex-column">
                <TimeGrid>
                    <GridItem grid={rowAt(1, 1)} style={{ borderBottom: '1px solid #efefef', fontSize: '12px', alignSelf: 'start', justifySelf: 'start' }}>2. 通常時間を入力してください</GridItem>
                    {
                        WEEK_DAYS.map((d, i) => {
                            return (
                                <GridItem 
                    ...
                    <GridItem grid={rowAt(2, 1)} style={{alignSelf: 'start', justifySelf: 'start'}}>
                        <div className="z3-flex" style={{'--gap': '4px'}}>
                            <span>AM</span>
                            <TimeInput name='regularAmStart' value={timeTable.regularAmStart} onChange={handleRegularTimeChange} onBlur={applyRegularTime} />
                            <span>&#65374;</span>
                            <TimeInput name='regularAmEnd' value={timeTable.regularAmEnd} onChange={handleRegularTimeChange} onBlur={applyRegularTime} />
                        </div>
                    </GridItem>
                    {
                    ...
                    <GridItem grid={rowAt(3, 1)} style={{alignSelf: 'start', justifySelf: 'start'}}>
                        <div className="z3-flex" style={{'--gap': '4px'}}>
                            <span>PM</span>
                            <TimeInput name='regularPmStart' value={timeTable.regularPmStart} onChange={handleRegularTimeChange} onBlur={applyRegularTime} />
                            <span>&#65374;</span>
                            <TimeInput name='regularPmStart' value={timeTable.regularPmEnd} onChange={handleRegularTimeChange} onBlur={applyRegularTime} />
                        </div>
                    </GridItem>
                    {
                        timeTable.weekDays.map((t, i) => {
            ...
            <div className="z3-flex-column" style={{'--gap': '4px'}}>
                <span className='w3-small'>3. 個別に時間を変更する場合は、該当する曜日を右クリックしてください</span>
            </div>
            {
                timeTable.weekDays.filter(x => isIrregular(x, timeTable)).length > 0 &&
                <div className="z3-flex-column" style={{'--gap': '4px'}}>
                    <div className="z3-flex">
                        <span>*</span>
                        {
                            timeTable.weekDays.filter(x => isIrregular(x, timeTable)).map((t, i) => {
                                return (
                                    <div key={i}>
