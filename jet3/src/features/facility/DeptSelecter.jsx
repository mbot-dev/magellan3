    ...
    switch (action.type) {
      ...
      case "setCheck": {
        ...
        if (checked && !draft.selections.find((x) => x.code === dept.code)) {
          ...
          !checked &&
          draft.selections.find((x) => x.code === dept.code)
        ) {
...
const DeptSelecter = ({ depts, onCancel, onSave }) => {
  ...
  const [{ selections, allDepts, isFilled, isDirty }, localDispatch] =
    useReducer(reducer, initialState);

  useEffect(() => {
    const asyncGet = async () => {
      try {
        const results = await margaret.getApi("master").getAllDepts();
      ...
      <div className="w3-border w3-padding-small">
        <div className="z3-auto-fit-grid" style={{ "--min-width": "220px" }}>
          {allDepts
            ...
            .map((d, i) => {
              const checked =
                selections.find((x) => x.code === d.code) || false; // Needs!
              return (
                <div
                  key={i}
                  className="z3-flex"
                  style={{ "--align": "center", "--gap": "4px" }}
                >
                  <input
                    type="checkbox"
                    className="w3-check"
