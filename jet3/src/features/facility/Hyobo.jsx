...
const Hyobo = () => {
  ...
  useEffect(() => {
    ...
    const asyncGet = async (fcId) => {
      try {
        const result = await margaret.getApi("facility").getTimeSchedule(fcId);
  ...
  const handleSave = (newDepts) => {
    ...
    const asyncUpdate = async (fc_id, attrs) => {
      try {
        await margaret.getApi("facility").updateFacility(fc_id, attrs);
  ...
  const handleSaveTable = (newTable) => {
    const asyncUpcert = async (newTable) => {
      try {
        await margaret.getApi("facility").replaceTimeSchedule(newTable);
      ...
      <Layout>
        <Main>
          ...
          <Control>
            ...
            {timeMode === "edit" && (
              <TimeEditor
                tableToEdit={timeTable}
                saveTable={triggerSave}
...
const Main = styled.div`
  display: grid;
  grid-template-columns: [labels] minmax(100px, 0) [controls] 1fr [cmd] minmax(
      100px,
      0
    );
  grid-auto-flow: row;
