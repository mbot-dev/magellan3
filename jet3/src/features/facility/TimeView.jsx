...
import { getAMSimbol, getPMSimbol, irregularInfo, isIrregular, WEEK_DAYS } from "./timeModel";
            ...
            <div className="z3-flex-column">
                <TimeGrid>
                    <GridItem grid={rowAt(1, 1)}/>
                    {
                        WEEK_DAYS.map((d, i) => {
                            return (
                                <GridItem key={i} grid={rowAt(1, i + 2)}>{d.disp}</GridItem>
                            );
                        })
                    }
                    ...
                    <div className="z3-flex" style={{'--gap': '16px'}}>
                        <span>{TEXT_HOLIDAY}</span>
                        {
                            timeTable.holidays.map((h, i) => {
                                return (
                                    <div key={i}>
                                        <span>{h.name==='sun' || h.name==='holi' ? h.disp : `${h.disp}${TEXT_WEEK_DAY}`}</span>
                                    </div>
                                );
                            })
                        }
