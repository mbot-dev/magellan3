...
const ProcedureSelection = () => {
  ...
  useEffect(() => {
    const asyncGet = async () => {
      try {
        const results = await margaret.getApi("input").getProcedureCatalogue();
  ...
  const handleSubmit = (e) => {
    ...
    const asyncUpdate = async (facilityId, payload) => {
      try {
        await margaret
          .getApi("stamp")
          .updateUsingProcedures(facilityId, payload);
      ...
      <MyForm onSubmit={handleSubmit}>
        <Scroller>
          <table className="w3-table w3-hoverable">
            ...
            <tbody>
              {allProcedures
                .filter((x) => x.entity !== "disease" && x.entity !== "input")
                .map((spec) => {
