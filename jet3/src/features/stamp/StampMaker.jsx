    ...
    const asyncGet = async (mode, text) => {
      ...
      try {
        if (mode === "prefix" || mode === "suffix") {
          results = await margaret.getApi("master").findDeseaseModifier(text);
        }
        if (mode === "body") {
          results = await margaret.getApi("master").findDeseaseByName(text);
        }
        if (mode === "icd10") {
          results = await margaret.getApi("master").findICD10ByCode(text);
        }
        setData(results);
      } catch (err) {
