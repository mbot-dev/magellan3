  ...
  return (
    <Dropdown>
      <DropdownBtn
        ...
        disabled={disabled}
      >
        {title}<MdArrowDropDown size='20px' />
      </DropdownBtn>
      <div ref={ref}>
        <DropdownContent
          className="w3-white w3-card w3-round-small"
          style={{ '--len': options.length > MAX_ITEMS ? MAX_HEIGHT : '', '--overFlowY': options.length > MAX_ITEMS ? 'scroll' : 'hidden', '--disp': isOpen ? 'block' : 'none' }}>
          {
            options.map((op, index) => {
              ...
              return separator ? (
                ...
                <MenuButton
                  key={key}
                  type='button'
                  className="w3-padding-small w3-round-small w3-hover-light-gray"
                  data-key={index}
...
export default SmallClickDropdown;
