...
class Context:
    ...
    def is_procedure(self, p):
        return True if p.get('code').startswith('1') and len(p.get('code')) == 9 else False
    
    def is_medicine(self, m):
        return True if m.get('code').startswith('6') and len(m.get('code')) == 9 else False
    
    def is_material(self, m):
        return True if m.get('code').startswith('7') and len(m.get('code')) == 9 else False
    ...
    def setup(self):
        ...
        try:
            ...
            # 休診日
            for h in holidays:
                test = self.week_day_dict.get(h.get('name'))  # h[name]=mon, tue, wed, thu, fri, sat
            ...
            for w in week_days:
                if w.get('day') == created_weekday:
                    self.amst = datetime.strptime(w.get('am_start'), '%H:%M').time() if w.get('am_start') != '' else None   # 09:00
                    self.amed = datetime.strptime(w.get('am_end'), '%H:%M').time() if w.get('am_end') != '' else None  # 13:00
                    self.pmst = datetime.strptime(w.get('pm_start'), '%H:%M').time() if w.get('pm_start') != '' else None  # 15:00
                    self.pmed = datetime.strptime(w.get('pm_end'), '%H:%M').time() if w.get('pm_end') != '' else None  # 18:30
                    break
    ...
    def calculate_age(self):
        ...
        return created_date.year - birth_date.year - ((created_date.month, created_date.day) < (birth_date.month, birth_date.day))
    
    def time_from_datetime(self, dt):
        return dt.time().replace(second=0, microsecond=0)
    
    def return_time_from(self, hours, minutes):
        return datetime.now().time().replace(hour=hours, minute=minutes, second=0, microsecond=0)
    ...
    def 外来管理(self):
        ...
        for b in self.karte.get('p'):
            procedures += [p for p in b.get('claim_items') if self.is_procedure(p) and p.get('c56_外来管理加算区分') == '1']
    ...
    def 向精神多剤(self):  #2
        ...
        for b in self.get_orals():
            med += [m for m in b.get('claim_items') if self.is_medicine(m) and m.get('koseishin') == 5]
        return len(med) >= 3
    ...
    def 特処2(self):  #2
        ...
        over_28 = len([b for b in orals if len(b.get('quantity')) >= 28 and b.get('unit') == '日分']) > 0
    ...
    def bv(self):
        return len([p for p in self.get_procedures('600') if p.get('c50_検査等実施判断区分') == '1']) > 0
    
    def 判尿(self):
        return len([p for p in self.get_procedures('600') if p.get('c51_検査等実施判断グループ区分') == '1']) > 0

    def 判血(self):
        return len([p for p in self.get_procedures('600') if p.get('c51_検査等実施判断グループ区分') == '2']) > 0

    def 判生1(self):
        return len([p for p in self.get_procedures('600') if p.get('c51_検査等実施判断グループ区分') == '3']) > 0

    def 判生2(self):
        return len([p for p in self.get_procedures('600') if p.get('c51_検査等実施判断グループ区分') == '4']) > 0

    def 判免(self):
        return len([p for p in self.get_procedures('600') if p.get('c51_検査等実施判断グループ区分') == '5']) > 0

    def 判微(self):
        return len([p for p in self.get_procedures('600') if p.get('c51_検査等実施判断グループ区分') == '6']) > 0

    def 判遺(self):
        return len([p for p in self.get_procedures('600') if p.get('c51_検査等実施判断グループ区分') == '7']) > 0
    ...
    def 検査逓減(self):
        return len([p for p in self.get_procedures('600') if p.get('c52_逓減対象区分') == '1' and p.get('actual_count', 0) >= 1]) > 0
